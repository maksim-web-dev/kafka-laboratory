...
class OrderService(
    ...
    private val topicCreated = "03.orders.created"
    private val topicCancelled = "03.orders.cancelled"
    ...
    fun createOrder(userId: String, product: String, quantity: Int, totalAmount: Double): OrderCreatedEvent {
        ...
        // key = userId: всі замовлення одного user → одна партиція → гарантований порядок
        kafkaTemplate.send(topicCreated, userId, event).whenComplete { result, ex ->
            if (ex != null) {
                ...
                log.info("OrderCreated published → topic={}, key={}, partition={}, offset={}", meta.topic(), userId, meta.partition(), meta.offset())
            }
        }
    ...
    fun cancelOrder(orderId: String, userId: String, reason: String): OrderCancelledEvent {
        ...
        // key = userId: cancelled потрапляє в ту саму партицію що і created для цього user
        kafkaTemplate.send(topicCancelled, userId, event).whenComplete { result, ex ->
            if (ex != null) {
                ...
                val meta = result.recordMetadata
                log.info("OrderCancelled published → topic={}, key={}, partition={}, offset={}", meta.topic(), userId, meta.partition(), meta.offset())
            }
        }
    ...
    fun createBatchKeyed(request: BatchOrderRequest): List<OrderSendResult> {
        return (1..request.count).map { i ->
            ...
            // .get() — синхронне очікування: результат містить реальні partition і offset
            val result = kafkaTemplate.send(topicCreated, request.userId, event).get()
            val meta = result.recordMetadata
            log.info("OrderCreated published → topic={}, key={}, partition={}, offset={}", meta.topic(), request.userId, meta.partition(), meta.offset())
            OrderSendResult(orderId = event.orderId, userId = request.userId, key = request.userId, partition = meta.partition(), offset = meta.offset())
        }
    }
    ...
    fun createBatchNoKey(request: BatchOrderRequest): List<OrderSendResult> {
        return (1..request.count).map { i ->
            ...
            val result = kafkaTemplate.send(topicCreated, null, event).get()
            val meta = result.recordMetadata
            log.info("OrderCreated published → topic={}, key=null, partition={}, offset={}", meta.topic(), meta.partition(), meta.offset())
            OrderSendResult(orderId = event.orderId, userId = request.userId, key = null, partition = meta.partition(), offset = meta.offset())
        }
    }
}